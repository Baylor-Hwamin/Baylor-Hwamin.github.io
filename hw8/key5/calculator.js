// this gives us the order of the buttons, which we can use to step through the
// buttons in various directions since we know the layout, + 1 moves to the next
// item, -1 previous, +4 is one row down, -4 is one row up
buttonOrder = [
    "#button7", "#button8", "#button9", "#buttonDivide", "#button4", "#button5",
    "#button6", "#buttonMultiply", "#button1", "#button2", "#button3",
    "#buttonAdd", "#button0", "#buttonClear", "#buttonEquals", "#buttonSubtract"
];
width = 4;

// add the selected class to an item. you can pass this any jquery selector,
// such as #id or .class calling this will de-select anything currently selected
...
// but you could rewrite this to return a list of items if you wanted to track
// multiple selections
...
// this uses the array buttonOrder to know the order of the buttons. so you
// could change buttonOrder to change the order that controls are highlighted/
// if no button is currently selected, such as when the page loads, this will
// select the first item in buttonOrder (which is the 7 button) selectNext: go
// to the right, wrapping around to the next row selectPrevious: go to the left,
// wrapping around to the previous row selectUp: select the item above
...
// handle calculator functions. all buttons with class calcButton will be
// handled here
    ...
    if (digits.indexOf(buttonLabel) != -1) {
        ...
        if (addingNumber) {
            ...
            if (firstValue == null) {
                ...
                addingNumber = false;
                // do we have a number on the stack already? if so, this is the
                // same as equals
